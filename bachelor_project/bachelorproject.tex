...
\abstract {
Security remains a foundational concern in software development, particularly as systems grow more complex and resource-constrained platforms persist in use. Among the most severe classes of vulnerabilities is arbitrary code execution (ACE), which allows attackers to manipulate a program’s behavior by executing unintended instructions, often without injecting new code, through control-flow or memory corruption.

This thesis explores ACE through a detailed case study of Pokémon Emerald, a legacy Game Boy Advance title, where in-game glitches can be chained to corrupt memory and redirect execution. By examining the mechanics of the \textit{Pomeg glitch} and \textit{Glitzer Popping}, the study shows how player-controlled inputs can be repurposed to craft valid ARM instructions that run directly on legitimate hardware, enabling full control over game behavior.

To complement the practical case study, the thesis also reviews state-of-the-art techniques for addressing ACE in modern systems. Detection methods are analyzed through the lens of machine learning, focusing on the use of Long Short-Term Memory (LSTM) networks to identify code patterns associated with remote code execution in PHP. On the prevention side, the thesis surveys compiler- and runtime-level strategies, including G-Free and EC-CFI, which enforce control-flow integrity and eliminate exploitable code paths.

Together, the case study and literature review illustrate how ACE remains relevant across different execution environments, and how both proactive detection and runtime enforcement are necessary to mitigate its impact.
...
\end{figure}
\begin{figure}[h!]
	\center{\includegraphics[width=0.25\textwidth]
		{figures/deca_hatch.png}}
...
Initially, in Chapter \ref{sec:today}, the focus is on detection, specifically through an analysis of the paper "Detection of Remote Code Execution Vulnerability in Website Source Codes Using LSTM Machine Learning Model" by Armin Zakarian and Muhammad Rahmani, published in 2024.\footnote{https://www.researchgate.net/publication/381650864\_Detection\_of\_Remote\_Code\_Execution\_vulnerability\_in\_website\_source\_codes\_using\_LSTM\_machine\_learning\_model} The paper addresses the detection of Remote Code Execution (RCE) vulnerabilities in web applications, a form of ACE that allows attackers to execute unintended code on a target system remotely.
